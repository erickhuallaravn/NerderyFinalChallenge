...
import { OrderHeaderStatus, RowStatus, UserType } from 'generated/prisma';
...
export class OrderHeaderService {
  ...
  async updateOrder(
    ...
    if (authPayload.userType !== UserType.MANAGER && order.customerId !== authPayload.customerId)
      throw new ForbiddenException('Access denied to this order');
  ...
  async anulateOrder(authPayload: JwtPayload, orderId: string) {
    ...
    return this.prisma.orderHeader.update({
      ...
      data: {
        ...
        statusHistory: {
          updateMany: { where: {}, data: { status: OrderHeaderStatus.ANULATED } },
        },
      },
    });
  }
